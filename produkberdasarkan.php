...
$data_produk = mysqli_query($koneksi, "SELECT * FROM produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE nama_kategori = '$katakunci'");
...
if (isset($_GET["urut"])) {
    $urut = $_GET["urut"];
    if ($urut == "Tertinggi") {
        $produk = show("SELECT * FROM produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE nama_kategori = '$katakunci' ORDER BY harga DESC LIMIT $halaman_awal, $batas_halaman");
    } else if ($urut == "Terendah") {
        $produk = show("SELECT * FROM produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE nama_kategori = '$katakunci' ORDER BY harga ASC LIMIT $halaman_awal, $batas_halaman");
    } else if ($urut == "Terbaru") {
        $produk = show("SELECT * FROM produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE nama_kategori = '$katakunci' ORDER BY id_produk DESC LIMIT $halaman_awal, $batas_halaman");
    }
} else {
    $produk = show("SELECT * FROM produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE nama_kategori = '$katakunci' ORDER BY id_produk DESC LIMIT $halaman_awal, $batas_halaman");
}

$jumlah_produk = show("SELECT COUNT(*) AS jml_produk FROM produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori  WHERE nama_kategori = '$katakunci'");
                            ...
                            <div class="col-6">
                                <div class="card card-produk h-100 rounded-16">
                                    ...
                                    <form action="" method="post" class="d-flex">
                                        ...
                                        <button type="submit" name="tambah_keranjang" class="btn btn-green w-100 fs-14 m-3 d-none d-md-block">
                                            Tambah ke Keranjang
                                        </button>
                                        <button type="submit" name="tambah_keranjang" class="btn btn-green w-100 fs-14 m-3 d-block d-md-none">
                                            + Keranjang
                                        </button>
                                    </form>
