...
$data_pesanan = mysqli_query($koneksi, "SELECT * FROM pesanan LEFT JOIN user ON pesanan.id_user = user.id_user WHERE status = 'Dikemas' ORDER BY id_pesanan DESC");

$jumlah_data = mysqli_num_rows($data_pesanan);
$total_halaman = ceil($jumlah_data / $batas_halaman);
$nomor = $halaman_awal + 1;

$pesanan = show("SELECT * FROM pesanan LEFT JOIN user ON pesanan.id_user = user.id_user WHERE status = 'Dikemas' ORDER BY id_pesanan DESC LIMIT $halaman_awal, $batas_halaman");
?>
...
<head>
    ...
    <title>Pesanan Dikemas - Macro Store Computer</title>
</head>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Pesanan Dikemas</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                            ...
                            <div class="card h-100 p-lg-4 p-2 border-1 shadow-1 rounded-16 mb-3">
                                <div class="table-responsive">
                                    <table class="table table-borderless table-hover fs-15 border-bottom">
                                        ...
                                        <?php else : ?>
                                            <tr>
                                                <td colspan="9" class="text-center py-4">Tidak ada pesanan dikemas.</td>
                                            </tr>
                                        <?php endif ?>
                                    </table>
                                    <!-- Pagination -->
