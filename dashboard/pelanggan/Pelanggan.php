...
$user = show("SELECT * FROM user WHERE role = 'user' LIMIT $halaman_awal, $batas_halaman");
?>
...
<head>
    ...
    <title>Pelanggan - Macro Store Computer</title>
</head>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-borderless table-hover fs-15 border-bottom">
                                        <tr class="table-light border-bottom">
                                            ...
                                            <th class="fw-500 text-dark py-3">Jenis Kelamin</th>
                                        ...
                                        <?php foreach ($user as $item) : ?>
                                            <tr>
                                                ...
                                                <td><?php echo empty($item["jenis_kelamin"]) ? '-' : $item["jenis_kelamin"] ?></td>
                                                <td><?php echo empty($item["nomorhp"]) ? '-' : $item["nomorhp"] ?></td>

                                                <!-- <td colspan="">
                                                        <a href="editPesanan.php?kode_pesanan=<?= $item["kode_pesanan"] ?>" class="btn btn-gray"><i class="bi bi-eye"></i></a>
                                                        <a href="hapus.php?kode_pesanan=<?= $item["kode_pesanan"] ?>" onclick="return confirm('Yakin hapus?')" class="btn btn-gray"><i class="bi bi-trash"></i></a>
                                                    </td> -->
                                            </tr>
                                            <?php $nomor++ ?>
                                        <?php endforeach ?>
